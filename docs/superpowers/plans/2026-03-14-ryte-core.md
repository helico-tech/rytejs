...
/** Koa-style middleware function — receives context and a next callback. */
export type Middleware<TConfig extends WorkflowConfig, TDeps> = (
	ctx: any,
	next: () => Promise<void>,
) => Promise<void>;

/** Terminal handler function — receives context, no next. */
export type Handler<
...
Note: The `Middleware` and `Handler` types use `any` for the context parameter here because the full `Context` type is defined in `context.ts` (circular dependency). Consumers who need fully typed middleware/handler variables should use inline annotations or let TypeScript infer from usage. These types exist so consumers can reference the shape.
...
## Chunk 4: Core Library — Router

### Task 8: Implement workflow router
...
### Task 9: Order fulfillment integration test
...
### Task 10: Content publishing integration test
...
### Task 11: Set up Husky git hooks
...
### Task 14: Write documentation content
...
### Task 15: README and publishing guide
